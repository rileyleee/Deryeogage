...
function UserInfo() {
  ...
  useEffect(() => {
    const ProfileData = async () => {
      try {
        const token = localStorage.getItem("accessToken");
        const response = await axios.get(`${REACT_APP_API_URL}/users/profile/mypage`, {
          headers: {
  ...
  return (
    <S.UserInfoWrap className="row">
      <div>
        <S.NickName>안녕하세요 <S.NickName className="nickname">{nickname}</S.NickName>님 !</S.NickName>
      </div>
      <div className="col-6">
        <br />
        <div className="container">
          <div className="row">
            <div className="col-4">
              {userInfo.profilePic && <S.ProfileImage src={userInfo.profilePic} alt="프로필 이미지" />}
            </div>
            <div className="col-8 d-flex flex-column justify-content-between">
              <div className="d-flex justify-content-between">
                <S.UserInfoText>닉네임 : {nickname}</S.UserInfoText>
                <S.UserInfoText>칭호 : {userInfo.simulationTitle ? userInfo.simulationTitle : "없음"}</S.UserInfoText>
              </div>
              <div className="d-flex justify-content-between">
                <S.UserInfoText>입양 : {userInfo.adoptToCount}</S.UserInfoText>
                <S.UserInfoText>분양 : {userInfo.adoptFromCount}</S.UserInfoText>
              </div>
              <S.UserInfoText>사전 테스트 점수 : {userInfo.preTestScore}</S.UserInfoText>
        ...
        <S.ModalLocation className="d-flex justify-content-center align-items-center">
          <S.ModalStyle>
            <div className="d-flex justify-content-end">
              <S.ModalClose onClick={() => setShowEditModal(false)}>X</S.ModalClose>
            </div>
